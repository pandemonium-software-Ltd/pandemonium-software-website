  ...
  for (const prospect of liveCustomers) {
    try {
      ...
      if (!res.ok) {
        ...
        console.error(
          `[digest:${tickId}] ${prospect.name} (${prospect.email}): Resend ${res.status} — ${errText.slice(0, 200)}`,
        );
        continue;
      }
      ok++;
      const headline = payload.hasActivity
        ...
        : "quiet month";
      console.log(
        `[digest:${tickId}] sent to ${prospect.email} — ${headline}`,
      );
    } catch (e) {
      failed++;
      console.error(
        `[digest:${tickId}] ${prospect.name} (${prospect.token}): ${e instanceof Error ? e.message : String(e)}`,
      );
    }
  }
