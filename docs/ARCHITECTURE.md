...
- C2 (DONE & LIVE): Cloudflare automation, end-to-end. Three
  sub-commits ship the full §4.3 Step 1 + Step 2.A flow:
  ...
  - **C2.2 (DONE & LIVE)**: Step 2.A part 1 — zone create +
    ...
    error paths (3).
  - **C2.3 (DONE & LIVE)**: Step 2.A part 2 — per-customer Worker
    provisioning + Custom Domain binding + HTTP verify. Extends
    step2-domain.run() with three new phases (D, E, F):
    - **D**: when zone is active and no `Worker Name` yet, upload
      a per-customer placeholder Worker named `mf-<8-hex-prefix>`
      (per §10 locked-in convention) via PUT
      /accounts/{id}/workers/scripts/{name} with multipart body
      (metadata + module). Source generated by
      `src/ops-worker/placeholder-worker.ts` — a self-contained
      ES module that returns a tiny styled HTML page ("Your new
      website is being built — back soon") with the customer's
      name HTML-escaped and baked in. Stamp `Worker Name` latch.
    - **E**: bind apex + www to the per-customer Worker via POST
      /accounts/{id}/workers/domains with
      `{ hostname, service: "mf-...", zone_id, environment:
      "production" }`. listWorkerCustomDomains first for
      idempotency (don't re-bind hostnames already bound). No
      latch — checked dynamically each tick because TLS
      provisioning takes a few minutes (status: pending → active).
    - **F**: when both bindings show non-pending status, fetch
      `https://<domain>/` (redirect: manual). On HTTP 200-399,
      stamp `Site Live At` latch. On 5xx (TLS still
      provisioning) or network throw, log to audit notes and
      retry next tick — no exception fires.
    Two new Notion columns: Worker Name (text), Site Live At
    (date). 13 new unit tests (32 total in step2-domain.test.ts):
    Phase D (4), Phase E (2), Phase F (5), shouldRun extensions
    (2). All HTTP fetches mocked via `vi.spyOn(globalThis,
    "fetch")`; all Cloudflare API calls mocked via vi.mock.
    Bundle: 165.92 KiB / 31.05 KiB gzip. Deployed and verified
    (version ca5fb21a-2526-4936-afba-ebebd503fcbd) — cron ticks
    cleanly, H1 SmokeTest prospect's behaviour unchanged
    (still gated on step1 not yet having a real CF invite).
